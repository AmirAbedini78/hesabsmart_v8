...
abstract class CustomFileRepository implements Countable, RepositoryInterface
{
    use Macroable;
    ...
    public function scan()
    {
        ...
        if ($this->app->has('tenant')) {
            $tenantModules = $this->app->make('tenant')->modules()->get()->groupBy('is_enabled');
            $disabledModules = isset($tenantModules[0]) ? $tenantModules[0]->pluck('name')->toArray() : [];
            //            $enabledModules = $tenantModules[1] ?? [];
        } else {
            $disabledModules = [];
            //            $enabledModules = $this->app->make('modules')->all();
        }
