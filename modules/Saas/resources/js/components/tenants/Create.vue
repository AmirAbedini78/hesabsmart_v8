<template>
  <ISlideover
    ...
    :visible="visible"
    :title="title || t('saas::saas.title')"
    :ok-text="$t('core::app.create')"
    :ok-disabled="form.busy"
    static
    form
    @hidden="handleModalHiddenEvent"
    @submit="createUsing ? createUsing(create) : createTenant()"
...
const { originalSettings, isReady } = useSettings()
...
function onAfterCreate(data) {
  data.indexRoute = { name: 'tenants-index' }
...
async function checkActivatedModules() {
  const activatedModules = form.activate_modules
  let invalidModules = ''
  const lastKey = activatedModules.length - 1

  activatedModules.forEach((module, key) => {
    if (
      !settings.value[`${module}_licence_type`] ||
      settings.value[`${module}_licence_type`] !== 'Extended License'
    ) {
      invalidModules += lastKey === key ? module : module + ', '
    }
  })

  if (invalidModules) {
    await Innoclapps.confirm(
      `Module/s [${invalidModules}] does not have an extended license. Do you wish to continue?`
    )
  }
...
async function makeCreateRequest(actionType = null) {
  await checkActivatedModules()

  let tenant = await createResource(form).catch(e => {
